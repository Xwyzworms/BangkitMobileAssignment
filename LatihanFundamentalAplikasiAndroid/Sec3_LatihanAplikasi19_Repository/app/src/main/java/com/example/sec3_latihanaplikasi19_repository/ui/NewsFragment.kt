...
import com.example.sec3_latihanaplikasi19_repository.databinding.FragmentNewsBinding
import com.example.sec3_latihanaplikasi19_repository.data.Result
...
class NewsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (tabName == TAB_NEWS) {
            ...
            viewModel.getBookmarkedNews().observe(viewLifecycleOwner) { result ->
                binding?.progressBar?.visibility = View.GONE
              if ( result is Result.Success)  {
                val bookmarked = result.data
